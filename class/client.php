...
public function  __construct ()
{
	...
	{
		if(isset($_SESSION['email'])){$this->email=$_SESSION['email'];} else{$this->email=NULL;}
		if(isset($_SESSION['password'])){$this->password=$_SESSION['password'];} else{ $this->password= NULL ;}
...
public function if_user()
{
	...
	{
		$this->config->query="select * from etudient where email='$this->email' and password='$this->password'";
		$this->config->excute();
...
{
	// get count email;
	$query_count="SELECT count(*) as count FROM etudient where email='$this->email'";
	//gadd new client
	$query_add="
		INSERT INTO `etudient` (`id`, `f_name`, `l_name`,`date_n`,`adress`,`email`,`password`,`phone`,`niveau`,`sexe`) 
